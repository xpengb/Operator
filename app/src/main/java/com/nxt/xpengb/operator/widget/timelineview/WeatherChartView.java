package com.nxt.xpengb.operator.widget.timelineview;
...
import com.nxt.xpengb.operator.util.SimpleSubscriber;
...
public class WeatherChartView extends LinearLayout {
    ...
    private List<HeWeather.DailyForecastBean> dailyForecastList = new ArrayList<>();
    ...
    private void letItGo() {
        ...
        HeWeather.DailyForecastBean yesterday = WeatherUtil.getInstance().getYesterday();
        if (yesterday != null) {
        ...
        for (int i = 0; i < dailyForecastList.size(); i++) {
            ...
            WeatherUtil.getInstance().getWeatherDict(dailyForecastList.get(i).getCond().getCode_d()).observeOn(AndroidSchedulers.mainThread()).subscribe(new SimpleSubscriber<WeatherBean>() {
                ...
                public void onNext(WeatherBean weatherBean) {
                    Glide.with(getContext()).load(weatherBean.getIcon()).into(ivIcon);
                }
            });
    ...
    public void setWeather(HeWeather weather) {
